...
export function TurnkeyCases({ namespace = "serviceTurnkey" }: { namespace?: string }) {
  const t = useTranslations(namespace);
  const pt = useTranslations("projects");
  const sectionRef = useRef<HTMLElement>(null);
  const isInView = useInView(sectionRef, { once: true, amount: 0.05 });
  const allItems = pt.raw("items") as CaseItem[];
  const items = allItems.filter((item) => featuredSlugs.includes(item.slug));
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0 opacity-[0.35] mix-blend-overlay">
        <svg width="100%" height="100%">
          ...
          <rect width="100%" height="100%" filter="url(#tk-cases-noise)" />
        </svg>
      </div>
                ...
                >
                  ...
                  >
                    <span
                      ...
                      style={{
                        marginBottom: "clamp(0.25rem, 1cqi, 0.5rem)",
                      }}
                    >
                      {item.category}
                    </span>
