...
export function TechnicalEquipment({ namespace }: TechnicalEquipmentProps) {
    ...
    >
      <div className="absolute inset-0 pointer-events-none z-0 opacity-[0.08] mix-blend-multiply">
        <svg width="100%" height="100%">
          <filter id="tk-equipment-noise">
            <feTurbulence type="fractalNoise" baseFrequency="0.65" numOctaves="4" stitchTiles="stitch" />
          </filter>
          <rect width="100%" height="100%" filter="url(#tk-equipment-noise)" />
        </svg>
      </div>
      ...
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-fluid-sm">
          {items.map((item, i) => (
            <motion.div
              key={item.title}
              initial={{ opacity: 0, y: 30 }}
            ...
            >
              <span
                className="font-heading text-primary/30 leading-none block"
                style={{ fontSize: "clamp(2rem, 4vw, 3rem)", marginBottom: "clamp(0.75rem, 1.5vw, 1.25rem)" }}
              >
                0{i + 1}
              </span>
              <h3
