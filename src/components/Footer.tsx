...
function FooterAccordion({
  ...
  return (
    <div className="border-b border-white/10">
      ...
      >
        <span className="text-fluid-xs font-heading tracking-[0.3em] text-white/60">
          {title}
        </span>
        <ChevronDown
          className={cn(
...
export function Footer() {
  ...
  const contactInfo = (
    <ul className="flex flex-col gap-2.5">
      ...
      <li><span className="text-fluid-xs text-white/60">{t("schedule")}</span></li>
    </ul>
  );

  const socialLinks = (
    <div className="flex items-center gap-3">
      <a href="https://www.facebook.com/mediashowgrup" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-10 h-10 flex items-center justify-center border border-white/[0.08] bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.06] transition-all">
        <svg className="w-4 h-4 text-white/40" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
      </a>
      <a href="https://www.instagram.com/mediashowgrup" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-10 h-10 flex items-center justify-center border border-white/[0.08] bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.06] transition-all">
        <svg className="w-4 h-4 text-white/40" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
      </a>
    </div>
  );
    ...
    <footer className="relative z-10 bg-[#0a0a0a] border-t border-white/10">
      <Container>
          ...
          {isMobile ? (
            <div>
              ...
              <div className="mb-6">
                ...
                />
                <p className="text-fluid-xs text-white/60 max-w-[260px]">
                  {t("tagline")}
                </p>
              </div>
              ...
              <div className="pt-6">
                <span className="text-fluid-xs font-heading tracking-[0.3em] text-white/60 block mb-4">
                  {t("social")}
                </span>
                {socialLinks}
              </div>
            ...
            <div className="grid grid-cols-10 gap-fluid-md">
              ...
              <div className="col-span-2">
                ...
                />
                <p className="text-fluid-xs text-white/60 max-w-[200px]" style={{ marginBottom: "var(--space-md)" }}>
                  {t("tagline")}
                </p>
                {socialLinks}
              </div>
              ...
              {/* Services */}
              <div className="col-span-2">
                <span className="text-fluid-xs font-heading tracking-[0.3em] text-white/60 block" style={{ marginBottom: "var(--space-sm)" }}>
                  {t("services")}
                </span>
                {servicesLinks}
              </div>
        ...
        <div className="border-t border-white/10 py-fluid-md flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-fluid-xs text-white/60">
            © {year} Media Show Grup. {t("rights")}
          </span>
          <div className="flex items-center gap-fluid-sm">
            <Link href="/privacy" className="text-fluid-xs text-white/60 hover:text-white/60 transition-colors">
              {t("privacy")}
            </Link>
            <Link href="/terms" className="text-fluid-xs text-white/60 hover:text-white/60 transition-colors">
              {t("terms")}
            </Link>
          </div>
        </div>
